namespace System.Windows.Controls
{
    ...
    internal class SelectionMultiple : InputSubscriberBase, ISelectionStrategy
    {
        ...
        private void SelectSingleItem2(TreeViewExItem item)
        {
            if ((IsShiftKeyDown && treeViewEx.SelectedItems.Count > 0 && !treeViewEx.IsVirtualizing) || IsControlKeyDown) return;
            List<object> itemsToUnSelect = new List<object>((IEnumerable<object>)treeViewEx.SelectedItems);
            if (itemsToUnSelect.Contains(item.DataContext))
            {
        ...
        private TreeViewExItem ItemToHandleInMouseClick(MouseButtonEventArgs e)
        {
            TreeViewExItem item = GetTreeViewItemUnderMouse(e.GetPosition(treeViewEx));
            if (item == null) return null;
            if (e.ChangedButton != MouseButton.Left && !(e.ChangedButton == MouseButton.Right && item.ContextMenu != null)) return null;
        ...
        {
            base.OnMouseDown(e);
            var item = ItemToHandleInMouseClick(e);
            if (item == null) return;
